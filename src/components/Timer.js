...
const PATTERN_ANDROID = [
  0 * ONE_SECOND_IN_MS,
  1 * ONE_SECOND_IN_MS, //vibras
  0.5 * ONE_SECOND_IN_MS,
  1 * ONE_SECOND_IN_MS, //vibra
  0.5 * ONE_SECOND_IN_MS,
  1 * ONE_SECOND_IN_MS, //vibra
  0.5 * ONE_SECOND_IN_MS,
  3 * ONE_SECOND_IN_MS, //vibra
];
...
export const Timer = ({ currentTask, clearTask }) => {
  ...
  const onEnd = (reset) => {
    const pattern = Platform.OS === 'android' ? PATTERN_ANDROID : PATTERN_IOS;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.clearTaskButtonWrapper}>
        <TouchableOpacity style={styles.button} onPress={clearTask}>
